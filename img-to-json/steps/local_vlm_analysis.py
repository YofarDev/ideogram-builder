...
_SYSTEM_PROMPT_RAW = _PROMPT_PATH.read_text().strip()
...
def _build_messages(user_text: str, system_prompt: str) -> list:
    return [
...
def analyze(image: Image.Image, verbose: bool = False, debug=None, model_name="Qwen3-VL-4B-Instruct-8bit", bbox_format: str = "xyxy") -> dict:
    ...
    if debug and debug.enabled:
        debug.save_text("02_vlm/system_prompt.txt", system_prompt)
    ...
    for attempt in range(2):
        ...
        if parsed is not None:
            ...
            for obj in parsed.get("objects", []):
                if "bbox" in obj:
                    # VLMs may return xyxy or yxyx depending on the prompt; normalize
                    # to Ideogram [y1, x1, y2, x2] regardless of what they emitted.
                    obj["bbox"] = to_yxyx(obj.get("bbox"), bbox_format)
            if debug and debug.enabled:
